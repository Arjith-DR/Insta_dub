...
from user.schemas import UserCreate, UserOut, UsernameChange, UsernameOut, PasswordChangeOut
...
async def change_username(user_id: int, username: UsernameChange):
    existing = await dispatch("username", "get_by_status", username.new_username)
    if existing:
        raise HTTPException(status_code=400, detail="Username already taken")

    updated = await dispatch("username", "change", user_id, username.new_username)
    if not updated:
        raise HTTPException(status_code=404, detail="User not found")
    return updated
